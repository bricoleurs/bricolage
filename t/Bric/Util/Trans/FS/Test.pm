...
sub test_uri_meths : Test(37) {
    ...
    for my $uri (
        ...
        my @parts = File::Spec::Unix->splitdir(unescape_uri($uri));
        my $fn = catdir @parts;
        ...
        my $base = escape_uri($parts[-1]);
        is $fs->uri_base_name($uri), $base,
            "uri_base_name('$uri') should return '$base'";

        pop @parts;
        my $trunced = catdir map { escape_uri $_  } @parts;
...
sub test_cat_uri : Test(7) {
    ...
    for my $parts (
        ...
        my $uri = catdir( map {
            map { escape_uri $_} File::Spec::Unix->splitdir( unescape_uri $_ )
        } @$parts );
