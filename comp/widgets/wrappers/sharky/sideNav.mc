...
foreach my $wf (@$workflows) {
    ...
    if ( $nav->{"workflow-". $wid} ) { # show open workflow
        ...
        $m->out(qq {<a href="#" onClick="return doNav('} . $r->uri . qq {?nav|workflow-${wid}_cb=0')">});
        ...
        $m->out(qq {<a href="#" class=sideNavHeaderBold onClick="return doNav('} . $r->uri . qq {?nav|workflow-${wid}_cb=0')">});
        $m->out(uc ( $wf->{name} )  . "</a>");
        $m->out("</td>\n</tr>");
...
% # desks
        ...
        $m->out("<a class=sideNavHeader href=" . $r->uri . "?nav|workflow-${wid}_cb=1>");
        $m->out(qq {<a href="#" onClick="return doNav('} . $r->uri . qq {?nav|workflow-${wid}_cb=1')">});
        $m->out(qq{<img src="/media/images/mdgreen_arrow_closed.gif" width=8 height=13 border=0 hspace=2></a>\n});
        $m->out(qq {<a href="#" class=sideNavHeader onClick="return doNav('} . $r->uri . qq {?nav|workflow-${wid}_cb=1')">});
