...
how a user's login session is maintained. AUTH_TTL is the length of time, expressed
in seconds, that an idle user remains logged in to Bricolage. That is, if a user
...
patient data in health care), and therefore preventing data from ever being sent
...
for authorizations. The disadvantage is that one cannot simply grant an individual
user permission to an individual object or group. One can, however, create
a user group with one member, and an object group with one member, and then
grant the single user group permission to access the single object group.
...
only be granted for objects in the "All *" series of groups, as an object is
automatically a member of its respective "All" group, but can't be added to any
other group until it has already been created. And finally, B<DENY> permission
...
access to the object. Likewise, if a user is granted CREATE and EDIT
...
So let's look at a few examples. Let us assume that we're working with a system
...
Celebrates Birthday" story. Bricolage determines that this story is a member of
...
the member of only one of these groups, "All Users", has just the one
...
Now let's look at "DrEvil." This fine user wishes to view the story "Black Hole
...
automatically have CREATE rights to every object in Bricolage, regardless of
other group memberships and permissions -- B<including DENY!> In fact,
permissions aren't even checked for members of this group - CREATE permission
is simply always assumed and granted. The name and permissions of the Global
Admins group cannot be edited, either - only the membership. Thus membership
in this group should be limited to just those users who are responsible for
administering the whole of the Bricolage application. Use the other Admin
groups to parcel out the rights to Administer particular parts of Bricolage.
...
Admins group. Then, in the side navigation bar, select ADMIN -> SYSTEM ->
Groups. You will be presented with a list of all the groups in the
system. Bricolage ships with only User groups, but if you look at the "Or Pick
a Type" select list, you can see that it supports all kinds of object
groups. (Note that the special "All *" groups are not editable.) Click "Edit"
to edit one of the user groups, say "Distribution Admins" (note that you must
have permission to edit groups to do this!). The interface provides fields to
edit the name and description of the group, plus a double list manager that
allows you to move objects (users, in this case) in and out of the group. The
double list manager allows you to assign members to the group.
...
permissions to grant to user groups to access the objects in the Distribution
Admins group. The "Pick a Permissions Type" select list at the top of the page
allows you to access three other sections of permissions. (Non-user groups
only have the one type of permission interface, for granting permission to
groups of users to access the members of the group you're editing.)
...
preferences, and remember that they are global and will affect how I<all>
users see the Permissions interface!
...
And finally, the "Desks" option allows you to grant permission to the members
of the current group (Distribution Admins) to access the documents on
particular desks. This feature is useful to allow certain groups of users to
access documents only on certain desks. It is also the means by which users
can be granted permission to CREATE new documents, by granting the CREATE
permission to the relevant start desk.

These last three selections will likely be your most used, but they appear
only when editing the permissions of user groups. The "User Groups" selection
is the only option in the permissions management screen for non-user group
...
setting will have been saved. Click "Save" again to keep any changes made to
...
the Global Admins group, meaning that that user could add himself to the
Global Admins group. But it doesn't seem like a good idea to allow any user to
be able to give himself higher permissions; only another user with those
higher permissions should be able to grant them to the first user.
...
grant to the group are limited to those he helself has to the members of the
...
has CREATE permission to the members, he can grant any permission. And, of
course, if he has no permission or DENY permission, he can grant no
permissions at all.

=back

The overarching rule of thumb on which these rules were based is that a user
can never grant any other user greater permissions than he herself has, either
through group membership or through permission changes.
...
Ability to READ, EDIT, or CREATE members of the group (depending on how high
a permission you want to allow them to grant to other user groups to access
