...
Authorization is the processt that checks the permissions on individual objects
before allowing users to access them. And encryption allows users to securly
...
that precaution would be compromized. We therefore recommend that you change
this value periodically -- say every couple of days or once a week.

=head1 ENCRYPTION

Bricolage uses the industry-standard Secure Socket Layer (SSL) for connection
encryption. Bricolage automatically uses SSL for all logins and for all user
profiles. By doing so, Bricolage assures that users' passwords are never sent
over the Internet in the clear. When users login to Bricolage, their passwords
are encrypted, and when users and administrators edit User profiles within the
application itself, the profiles are edited via SSL so as to ensure that any
password changes are also sent via encrypted channels.
...
genereral because encryption encurs a significant performance overhead. A lot of
processor time has to go into encryping and decrypting all data sent to and from
...
in the clear over the Internet. Bricolage makes it a simple proces to use SSL
...
unencypted connection. This is because Bric checks the status of the "Use SSL"
...
logout, and then log back in with the "User SSL" checbox unchecked.
...
aturhorization in detail, so that you can always return to this document if
you're having trouble.

=head2 How Authorization Works

In order to keep implementation as straight-forward as possible -- and therefore
to also save processing time during uthorization checking, we designed Bricolage
...
So the granularity can still be acheived, but without violating the elegance of
the model.
...
B<EDIT> permission allows uesrs to both view and edit the contents of objects
...
only be granted for objects in the "All *" series of groupes, as an object are
automatically members of its respective "All" group, but can't be addded to any
other group until they've alreay been created. And finally B<DENY> permission
...
crossing of permissions requires permission reconcilliation. The way multiple
...
questionk, it decides which one wins, following the criteria outlined above.
...
Another expamle. Let's say that user "Mcnibblet" wishes to edit the "Dubbya
Celebrates Birthday" story. Bricolage detemines that this story is a member in
...
DrEvil to access the "Black Hole Destroys Earth" story are READ, CREATE, EDITY,
...
the permissions were deterimined.
...
the membership. Thus memberhip in this group should be limited to just those
...
from the above dicussion, there are two slightly different kinds of groups, at
...
the name and descriptio of the group, plus a double list manager that allows you
to move objects (in this case users) in and out of the group. The doble list
...
granted to the the members of the group, plus the permisisons granted to other
gruops to access the members of this group. If you're looking at the "Alert Type
...
click the "READ" radio button for the "Media Procucers" row and then click
...
the preferene, simply edit the "Permissions Widget" profile in the Preferences
...
can grant CREATE access, incidentally) as well as all the other gruops in the
...
These first three sectcions of permissions will likely be your most used, but
...
permissions management screen for I<all> group profiles. We belive that this
...
group profiles invidividually to grant permissions on your new group.
...
$Log: Security.pod,v $
Revision 1.1  2001-09-06 21:52:53  wheeler
Initial revision
