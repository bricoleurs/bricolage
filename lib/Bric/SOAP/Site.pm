...
This method queries the database for matching sites and returns a
list of ids.  If no sites are found an empty list will be returned.
...
Set false to return deleted sites.
...
Specifies a single site_id to be retrieved.

=item site_ids

Specifies a list of site_ids.  The value for this option should be an
array of integer "site_id" assets.
...
A list of "site_id" integers for the assets to be updated.  These
...
sub is_allowed_param {
    ...
    my $allowed = {
        list_ids => { map { $_ => 1 } qw(name description domain_name active) },
...
sub load_asset {
    ...
    foreach my $adata (@{ $data->{$module} }) {
        ...
        $asset->set_domain_name($adata->{domain_name});
...
Serializes a single site object into a <site> site using
...
sub serialize_asset {
    ...
    $writer->dataElement(domain_name => $asset->get_domain_name);
