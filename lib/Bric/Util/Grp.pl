...
is( Bric::Util::Grp::Person->get_class_id, 9, "Person class class ID is 9" );
ok( ! Bric::Util::Grp->get_supported_classes, "Check Grp supported classes." );
...
ok( $class = Bric::Util::Grp->member_class, "Get grp member class" );
is( $class->get_key_name, 'bric', "Check grp member class key_name" );
ok( $class = Bric::Util::Grp::Person->member_class, "Get Person member class" );
...
ok( my $subclasses = Bric::Util::Grp->href_grp_class_keys, "Get subclasses" );
...
TODO: {
    ...
    # There are a few class methods in Grp.pm that have the same names as the
    # instance methods generated by Bric.pm. This can cause conflicts, and
    # this Grp is an instance of that conflict. In reality, however, it
    # doesn't seem to matter much. Still, it'd be good to fix one of these
    # days.
...
# Okay, now it's time to play with members. First, get a well-known group to
# play with.
ok( $grp = Bric::Util::Grp->lookup({ id => 22 }), "Get Prefs grp" );
...
ok( UNIVERSAL::isa($grp, 'Bric::Util::Grp::Org'), "Confirm org grp class" );
...
ok( $grp = Bric::Util::Grp->lookup({ id => $gid }), "Lookup org grp again" );
@mems = $grp->get_members;
