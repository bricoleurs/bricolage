...
This module handles burning business assets using HTML::Template formattingassets.
...
BEGIN {
    Bric::register_fields({
			   ...
			   # Private Fields
			   _template_root   => Bric::FIELD_NONE,
...
Creates a new Template burner object.  Takes a single parameters -
$burner which is a Bric::Util::Burner object.  The new object will
have its attributes initialized by the passed object.
...
Publishes an asset.  Returns a list of resources burned.  Parameters
are:
...
sub burn_one {
    ...
    my $path        = $fs->cat_dir($self->get_out_dir, 'oc_'. $oc->get_id, $base_uri);
    ...
    $self->_set([qw(story oc cat _output_filename _output_path _output_ext 
                    _res page _base_uri _at _header _footer)],
		[$story, $oc, $cat, $oc_filename, $path, $oc_ext, 
                 $res, $page, $base_uri, $at, $header, $footer]);
	...
	if (defined $category_output) {
	    ...
	    ($header, $footer) = split(/${\CONTENT}/, $category_output, 2);
...
This method finds the script for the given element and executes it.
The return value is the output of the script.

If a script cannot be found for the element then one is autogenerated
that looks like:

  return $burner->new_template()->output;

This enables you to write template files that contain just the default
template autofill template tags and not have to create a script file
to drive them.

An Apache::Registry-esque compilation is performed - the script source
is compiled as the body of a subroutine in a private package.  After
the first time a script is compiled it is cached in memory and only
re-compiled if changed.
...
sub run_script {
  ...
  die $ap->new({msg => __PACKAGE__ . "::run_script() requires an \$element argument."})
    unless $element;
  ...
  unless(defined $script) {      
      $script = "$template_root/__autogenerated__/" . _element_filename($element) . ".pl";
      $sub = "return \$burner->new_template()->output;";
  }
  ...
  if (exists $SCRIPT_CACHE{$package} and 
      $SCRIPT_CACHE{$package} eq md5($sub)) {
      # compiled code is still good - nothing to do
      print STDERR __PACKAGE__, "::run_script() : skipping compilation - cached copy still good.\n"
	  if DEBUG;      
  } else {
...
This routine returns a new HTML::Template::Expr object.  This method
can take all the same options as HTML::Template::Expr::new() (which
is, in turn, mostly the same as the options to HTML::Template::new())
with a few additions.  The additional options are:

=over 4

=item element

The element option tells the burner to find the template associated
with that object and open it.  Defaults to the current global
$element.  To turn off this option simply specify a template by name
using the filename option.

=item autofill

The returned template object will have TMPL_VARs and TMPL_LOOPs set
for the element's attributes.  You must specify the element option to
use autofill.  The autofill option defaults to 1.

For details about the TMPL_VARs and TMPL_LOOPs that autofill creates
see L<Bric::HTMLTemplate>.

=back

Additionally, some of the defaults for HTML::Template::new() are
different:
...
sub new_template {
    ...
    # load args
    die $ap->new({msg => "new_template called with odd number of arguments - args should be a list of key-value pairs"})
      if (@_ % 2);
    my %args = @_;
	...
	die $ap->new({ msg => "Unable to find HTML::Template template file (" . _element_filename($element) . ".tmpl)" })
	    unless defined $file;
...
This routine breaks pages in the output.  It returns a page boundary
marker to insert in into the text.
...
This routine returns the magic content marker used in category
templates to break between the header and the footer.
...
This method builds all the TMPL_VARs and TMPL_LOOPs that can be
extracted from $element.  Returns a hash-ref suitable for passing to
$template->param().
...
sub _build_element_vars {
	...
	print STDERR __PACKAGE__ . "::_build_element_vars : saw $name (", join(', ', @$path), ")\n"
	    if DEBUG;
	...
	unless ($e->is_container) {
		...
		push @{$loop{"$name\_loop"}}, { %{$self->_build_element_vars($e,
									     $template, 
									     [ @$path, 
									       "$name\_loop" 
									     ])},
...
Finds a file for this element and extension (.pl or .tmpl) in the
current oc and cat.  Searches up the category tree as necessary.
Returns undef if the file cannot be found.

As a special-case if $element eq "category" then the category script
or template is searched for.
...
sub _find_file {
    ...
    print STDERR __PACKAGE__, "::_find_file(", $element->get_name(), ", $extension)\n"
	...
	my $path = $fs->cat_dir($template_root, @cats, $filename);
...
sub _add_resource {
    ...
    # Set the media type.
    $res->set_media_type(Bric::Util::MediaType->get_name_by_ext($self->_get('_output_ext')));
    # Add our story ID.
...
Writes pages in $output (and _header and _footer) to the appropriate
output files on disk.  Also takes care of building a list of resources
in _res for the files written.
...
sub _write_pages {
	...
	for (my $page = 0; $page < @pages; $page++) {
	    ...
	    my $uri = $fs->cat_file($self->_get('_base_uri'), substr($filename, length($path)));	    	
	...
	print STDERR __PACKAGE__, "::_write_pages() : opening single page $filename\n"
	    if DEBUG;
	...
	# add resource object for this file
	$self->_add_resource($filename, $fs->cat_file($self->_get('_base_uri'), substr($filename, length($path))));
    }
}
...
Returns the filename for the given $page number - index.html for 1,
index1.html for 2, etc.
