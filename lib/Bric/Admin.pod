...
Install Apache 1.3.12 or greater with mod_perl, mod_rewrite and
mod_proxy.  For SSL also install mod_ssl.
...
=item Devel::Profiler 0.03

=item Apache::SizeLimit
...
Follow these instructions to prepare your system for Bricolage.  If
you're upgrading from an older version of Bricolage see the UPGRADING
section below.
...
C<make install> isn't necessary because the sources will be compiled into
Apache.

=head2 Apache Interface to OpenSSL (mod_ssl) (optional)

You'll need to install mod_ssl if you want to use Bricolage's SSL support to run
...
This command prevents incompatibilities between Apache's implementation of the
...
L<http://bricolage-bugzilla.thepirtgroup.com>.  Include a copy of all
*.db files generated by the Makefile as well as a complete description
of where the process failed.  Then, either wait for us to fix the
problem or try the manual installation instructions below.
...
But you can also configure Bricolage to run on a virtual host. See
<"Configuration"> below.
...
You'll need to adjust the psql line to match your PostgreSQL settings
...
Not only does it enable secure logons to the system, but it also allows
...
B<TEMP_DIR:> The system will create a sub-directory below this
directory where Bricolage will store temporary files.  Must be
writable by SYS_USER described above.  Defaults to the return value
from File::Spec::Unix::tempdir() if not specified.
...
manually install Net::SFTP (the installer doesn't currently support optional
modules). Furthermore, you should login to the remote server as Apache user
(e.g. "nobody") in order to initialize `~/.ssh/known_hosts' before using the
SFTP mover (you might also need to create the directory /.ssh with write
permission for "nobody", in case nobody's home directory is something like
`/bin/false' and ssh falls back to `/' as the home directory). Defaults to
...
user (often "nobody") like this:
   ...
   0 2 * * * /usr/local/bricolage/bin/bric_clean_tmp.pl
