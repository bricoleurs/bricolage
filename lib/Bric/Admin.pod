...
C<$BURN_ROOT/comp> if not specified.

=item *

B<BURN_DATA_ROOT:> The directory where Mason stores Templates once it has
compiled them. Defaults to C<$BURN_ROOT/data> if not specified. Make sure that
SYS_USER has permission to write to this directory.
...
B<WARNING:> The FTP server component is an experimental feature and has not been
fully tested of completely implemented!
...
B<FTP_LOG:> Set this to the file where you want to FTP logs written to. Defaults
to $BRICOLAGE_ROOT/ftp.log if not set.
...
editing templates. For more information see the documenation for
