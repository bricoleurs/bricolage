...
the Bricolage UI, simply download it from L<http://www.htmlarea.com/>,
decompress the files, and move them to the F<comp/media/htmlarea> directory in
your Bricolage root. You will also need to enable the C<ENABLE_HTMLAREA>
F<bricolage.conf> directive (see L<UI Configuration|/"UI Configuration">).
