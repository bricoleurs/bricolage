...
directory). Session files are stored in C<$TMP/bric_sesion> and
C<$TMP/bric_lock>. These directories should stay relatively free of cruft, as
the session files are regularly deleted when users logout or their login
sessions expire and they attempt to log back in. Still, it'd be good to set up a
cron job to monitor these directories and delete fiels and subdirectories that
...
C<$TMP/FileCache>. The files in this directory can be periodically deleted. Just
be sure to shut down Bricolage before doing so, and then you can start it up as
soon as they're gone. This will ensure that no one tries to access the cache
after it has been deleted, and it will be recreated when the server starts
again.
...
$Log: Admin.pod,v $
Revision 1.6  2001-10-02 16:23:59  samtregar
