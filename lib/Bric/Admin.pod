...
Checkout the tools repository from CVS (cvs checkout tools). It doesn't matter
where you check this out, since these are just utilities you'll use only for the
installation. [Change this later, once we've packaged up the code for actual
distribution].

=item *

Change directories into tools/bin and run pgimport as follows:
...
B<FULL_SEARCH:> If set to 1, the system will perform
sub-string searches by default.  This is more expensive but generally
what users expect.
...
Bricolage stores all of its applicatin files -- including the UI elements and
...
$Log: Admin.pod,v $
Revision 1.1  2001-09-06 21:52:46  wheeler
Initial revision
