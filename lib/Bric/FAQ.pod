...
There is a FreeBSD port for Bricolage 1.8. To use it, simply update your ports
tree, and then:
...
There is currently only a tentative start for a bricolage debian package, and
unfortunately, it is rather out of date. The location of the package is
L<http://people.debian.org/~erich/bricolage>.
...
POTS - L<http://www.pots.com.tw/>

=item *

Newzilla - L<http://www.newzilla.org/>
...
won't show up in the template select list. This behavior has to do with sites;
only story type and media type elements are associated with sites, so to
create a new template in a workflow (which is associated with a site), it
looks for all the story type and media type elements associated with that
site, and then adds all subelements of those elements. So unless your new
subelement is associated with a story type or media type element, you won't
see it when you go to create a new template.
