...
inactive in the database. Rhis prevents creating a new template with the same
name in them same category in the same output chanel. In a future release, an
...
F<bricoalge.conf> directive.

=item password

The password for your Bricolage PostgreSQL user.
