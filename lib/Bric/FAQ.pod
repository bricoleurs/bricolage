...
I'm going to go out on a limb here and guess that you're using Safari on Mac
OS X. Safari has a JavaScript bug wherein it doesn't know the name of a
window. The "Click here to start" link opens a new named window, but since
Safari doesn't know that it's named, Bricolage's JavaScript can't tell that
it's named, either, and so keeps trying to get you to open a window with
the correct name. It'll never happen. Apple should be aware of this issue,
as I've filed a bug report for it using Safari's bug reporting interface, and
have reported it again via Apple's official bug database (Bug # 3485193).
