...
L<http:E<sol>E<sol>www.eweek.comE<sol>article2E<sol>0,3959,652977,00.asp>
...
The PIRT Group - L<http:E<sol>E<sol>thepirtgroup.comE<sol>>

=item *

World Health Organization - L<http:E<sol>E<sol>www.who.intE<sol>>

=item *

DFA - L<http:E<sol>E<sol>dfafunds.comE<sol>>

=item *

Macworld - L<http:E<sol>E<sol>www.macworld.comE<sol>>

=item *

Ad Council - L<http:E<sol>E<sol>www.adcouncil.orgE<sol>>

=back

=head2 Installation

=head3 Why does mod_perl have to be statically compiled into Apache? I've been
using mod_perl as a DSO and have had no problems with it at all.
...
noticed. The Mason site (L<http:E<sol>E<sol>www.masonhq.comE<sol>>) has a
...
L<http:E<sol>E<sol>www.masonhq.comE<sol>docsE<sol>faqE<sol>#why_am_i_getting_segmentation_fa>

=item *

L<http:E<sol>E<sol>www.masonhq.comE<sol>docsE<sol>faqE<sol>#does_mason_leak_memory>

=back

You can also search the Mason Users list archives
(L<http:E<sol>E<sol>marc.theaimsgroup.comE<sol>?l=mason>) and the mod_perl
Users list
archives (L<http:E<sol>E<sol>marc.theaimsgroup.comE<sol>?l=apache-modperl>) for some
...
=head3 Why does C<make install> exit with the following error?

    Missing or empty inst/bricolage.sql!

You downloaded Bricolage from CVS and didn't
run `make dist`. Do something like this:
...
=head3 Why do I get an error like the following in Apache's F<error_log>?

    [error] access to / failed for 127.0.0.1, reason: No cookies found.

This only happens the first time your browser accesses Bricolage.
It is harmless and indicates that no Bricolage session cookie
has been set yet.

=head3 Why do I get a 404 when I connect to the Bricolage UI and get an
error like the following in Apache's F<error_log>?
...
=head3 When I try to login to Bricolage, I get redirected to F</login>, but
the page is blank. There's nothing in the HTML source except C<<
<html><head></head></html> >> or nothing at all. There are no errors in the
error log or the access log. What's wrong?
...
these circumstances, the browser had no HTML source at all. At any rate, the
problem was fixed by reinstalling Bricolage (a C<make upgrade> would probably
do the trick, too), but it's likely that simply copying these two files from
the Bricolage source directories to your installed component root will correct
the problem. Holler on the bricolage-general mail list if it doesn't.

=head2 Element Administration

=head3 I added a field to an element and deleted it, and now I can't add a new
...
See the "About" section of the online help.
...
usage, or hacking. The project's website is
L<http:E<sol>E<sol>www.bricolage.ccE<sol>>. The
website lists information about mailing lists.
...
F<README>,
L<http:E<sol>E<sol>bricolage.ccE<sol>>,
