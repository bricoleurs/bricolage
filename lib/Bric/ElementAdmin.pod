...
flattened out. We don't need to represent the hierarchy of Element relationhips
...
type are subsumed under their type. Field are in turn indented under the
...
however, you should leave this option unchcked.
...
Now the "Stories" Element Typs is defined similarly to the "Covers" Element
...
need to add subelement to your Story element that can be associated with media.
...
of theh Element Type Profile. Fill in the form fields as follows:
...
text area input field. Selec the radio button next to "Text Area". The input
...
Leave this field blank to allow values of unlimited length.
...
You'll not that this Element profile is different from the "Page" profile in
...
If the coveline is likely to often be the same, you may indeed wish to include a
default value.
...
Futhermore, our specification above requires no subelements or fields for this
...
Lists" Element Type. These, too, are simple, but each contain a Subelelement.
...
for the remaining column fields:
  ...
  Name          Type     Subelements
  ------------  -------  -----------------------------
  Column Two    Columns  Current Stories
...
You might even find it useful to sketch out what you expect your site content to
look like on your site and then using that sketch to decompose it into its basic
parts.
...
Revision 1.7  2001-09-27 14:36:36  wheeler
It's done, it's done!

Revision 1.6  2001/09/27 12:39:45  wheeler
I'm nearly done! Going to a meeting, but wanted to get the most recent changes
commited. It *will* be done today.

Revision 1.5  2001/09/25 10:12:33  wheeler
Checkpoint. Testing syncmail changes again (it should work now!).

Revision 1.4  2001/09/25 09:54:56  wheeler
Checkpoint -- testing changes to syncmail.

Revision 1.3  2001/09/25 09:28:26  wheeler
Checkpoint.
