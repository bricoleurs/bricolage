...
running C<VACUUM>. Running C<ANALYZE> (known prior to PostgreSQL 7.3 as
C<VACUUM ANALYZE>) also collects statistics representing the dispersion of the
...
when several query execution paths are possible. Running C<ANALYZE>
periodically will increase the speed of the database in processing user
queries.

We recommend that you C<VACUUM> and C<ANALYZE> on your Bricolage database
...
vacuum is successful, C<vacuumdb> will output "VACUUM". In the event of a
failure, it will output "vacuumdb: Vacuum failed."

However, the best way to keep your database well vacuumed is to use the
C<pg_autovacuum> utility that comes with PostgreSQL 7.4 and later (and which
can be used with PostgreSQL 7.3, as well). To quote the C<pg_autovacuum>
F<README> document:
...
The specific tuning recommendations below are for PostgreSQL versions 7.3 and
7.4. Tuning will be slightly different for version 7.2, and very different for
version 7.5 when it comes out. Consult the necessary documentation.
...
queries, so 8 MB should be okay. Unlike C<shared_buffers>, the C<sort_mem>
setting is in KB buffers, so setting it to 8 MB would be:

  sort_mem = 8192

The difficult issue with C<sort_mem> is that more is needed for bigger
queries, but C<sort_mem> is not shared, being allocated on a I<per-sort>
basis. You do not, B<ever>, want to use more cumulative C<sort_mem> than your
...
query. This means that you have to be prepared to have C<3 * sort_mem>
available for each concurrent query. Overall, this means (with fudge factors)
that when you can calculate the maximum C<sort_mem> it is completely safe to set
as: C<Available RAM / max_connections * 3>. But more than 12 MB C<sort_mem>
...
between C<max_connections> and C<sort_mem>, lowering this number can also help
you to find a middle ground between a healthy value for C<sort_mem> and
...
The C<vacuum_mem> directive reserves memory for C<vacuum>ing the database. If
your PostgreSQL server has a decent amount of RAM (say 1-2 GB), increase the
value of C<vacuum_mem> to allow C<vacuum>s to go faster. We recommend 64 MB
(set in KB):
...
newer, faster processors, the default setting of 11 for C<geqo_threshold>,
...
See Josh Berkus' article, "Tuning PostgreSQL for performance" at
L<http://www.varlena.com/varlena/GeneralBits/Tidbits/perf.html>. You can
search the archives of the pgsql-performance list at
L<http://archives.postgresql.org/pgsql-performance/> and subscribe to the list
at
L<http://webmail.postgresql.org/mj/mj_wwwusr?domain=postgresql.org&func=lists-long-full&extra=pgsql-performance>.
