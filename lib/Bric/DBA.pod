...
buffers that the database server can use. Each buffer is 8Kb. It's important
to remember that this is only a holding area, and I<not> the total memory
...
and number of concurrent queries. No one has yet reported positive results for
any number over 6000.
