...
allocation. On a Linux system, shared memory is controlled through settings in
the F</proc> filesystem. So, for our example, where we've decided to give
PostgreSQL up to 192 MB of shared memory, here's how you would make the kernel
setting (note that the kernel setting is in bytes, so our setting is
C<192 * 1024 * 1024 = 201326592>):

  $ echo 201326592 >/proc/sys/kernel/shmall
  $ echo 201326592 >/proc/sys/kernel/shmmax

Consult your OS's L<sysctl> man page for details on getting and setting shared
memory settings. Now you can update your F<postgresql.conf> file's shared
memory buffers setting to take advantage of this shared memory:
...
queries, so 32 MB should be okay. Like C<shared_buffers>, the C<sort_mem>
setting is in 8 Kb buffers:
...
database to determine the optimum setting. Since the amount of shared memory
that PostgreSQL will request from the OS is the value of C<max_connections *
shared_buffers>, lowering this number can also help you to find a middle
ground between a healthy value for C<shared_buffers> and needing to
reconfigure the kernel's SHMMAX (See L</"Shared Buffers">).
...
topic at L<http://www.lyris.com/lm_help/6.0/tuning_postgresql.html>. You can
