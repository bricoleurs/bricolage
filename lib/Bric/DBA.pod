...
database.  It's purpose is to reclaim storage and to collect
information for the optimizer.  B<VACUUM> opens every table in the
database, cleans out records from rolled back tables. Running B<VACUUM
...
There are two ways to <VACUUM ANALYZE> a database. The first is through an interactive
