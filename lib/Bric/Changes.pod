...
the only charset loaded in C<$LOAD_CHAR_SETS>. Reported by Daisuke Maki. 
[Paul Orrock]
...
More changes around bug 1267, remove protocol+hostname+port from 
practically all redirects (especially "user profiles") in order to 
allow reverse proxying to work in Apache2. [Steffen Schwigon]
...
Fix for bug 1254: checkbox fields are now saved correctly. [Scott]

=item *

Fix for bug 1294, where Search/Replace in Bulk Edit wasn't working. [Greg Heo]

=item *

Fix for bug 1269, needed to HTML-escape Notes. [Scott]
...
Fix bug 1264, prevent searching on bad dates (like February 31). [Greg Heo]
