...
The C<version_id> parameter to C<list()> and C<lookup()> has been changed to
C<instance_id>.  C<version_id> now refers to a specific version of an asset,
which is no longer the same as an instance (one version can have multiple
instances -- one for each IC) [Marshall]
