...
Added Bric::Util::Job::Pub (and bric_queued).  Publication can be placed into a
queue, and deferred until a scheduled time.  This along with the addition of a
bric_queue daemon or the use of a carefully tuned instance of Bricolage allow
the Bricolage administrator to control the amount of system resources given
over to publishing.  [Mark]
...
is pressed. [oE<atilde>o Pedro]

=item *

select_object.mc correctly saves its selected_id state so that,
for example, dropdown menus in New Story remember the item
