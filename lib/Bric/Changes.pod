...
Removed auto-generation of dhandlers in the Mason Burner. The C<dhandler_name>
parameter to HTML::Mason::Interp (passed through to HTML::Mason::Request) made
them unnecessary. Suggested by Dave Rolsky. [David]
...
pop up in the location field in my browser, but it does display proplerly if I
...
Its possible to create new stories again without running into errors saying
that a URI is not unique because the cover date and slug were accidentally
exclued from the URI. [David]

=item *

Fixed page logging so that redirects to the page before the current page can
work correctly. It was most noticably broken when trying to associate a
