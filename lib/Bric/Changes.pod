...
Added a timetamp for data that F<bric_queued> sends to its log. Thanks to Neal
...
from auotopopulating values in the future. Thanks to Marshall for the
...
or workspace. Thanks to Matt Rolf for the suggestiong (Bug #69). [David]
...
"Clear Values" button is clicked. Thanks to Matt Rol for the report (Bug #
74). [David]

=item *

The category autocompetion feature in the "New Story" profile now refuses to
submit if a non-existant category has been typed in. If the user somehow
...
making the Denison available to diagnose the problem (Bug #102). [David]
