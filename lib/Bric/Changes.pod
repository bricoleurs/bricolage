...
Added an " TraceEnable off " entry to the config in ApacheConfig.pm for Apache 1.3 
to neutralize the potentially malicious use of HTTP TRACE requests.
...
(my suggestion wasn't quite right). The handling of E<lt>publish_statusE<gt>
in bric_soap has been a bit dodgy. Also changed publish_status to deploy_status
in C<Bric::SOAP::Template>. [Scott]
...
Editing user prefs when SSL is enabled no longer breaks the User profile.
Reported by Scott (Bug#1339). [David]
...
warning to the error log and continues on its merry way. Suggested by Scott
(Bug #1358). [David]
