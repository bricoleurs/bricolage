...
Converted "Bulk Edit" into the UI to use the new POD syntax supported by the
element classes and elimionated the old "Super Bulk Edit" interface since it
...
Added $element object to exception object in throw_burn_error. Now a kind of
breadcrumb trail will show where an error occurred in a template. (Note: this
...
Fixed templates that display any elements whose template path is '/'
(Bric::Util::Trans::FS::split_uri was broken by the URI slash improvement
above). [Scott]
