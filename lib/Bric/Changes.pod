...
Emails sent by Bricolage now set the send date, since our understanding is
...
Fixed C<has_keyword> for stories and media, which was always returning
C<undef>. Spotted by Clare Parkinson. [Scott]
...
Publishing a published verson of a document that has a later version in
...
The C<has_keyword()> method in the story and media classes now works properly.
Reported by Clare Parkinson. [David]
...
events logged when a job is run via the UI are now also loged by
...
uploading media files. Repoted by Scott Lanning. [David]
...
The JavaScript that handles the double listmanager has been vastly optimized.
...
reverted version on the user's sandbox. Also, cancelling a checkout now
