...
A "Bulk Publish" link in ADMIN->PUBLISHING that lets members of the Global
Admins group publish story and media documents by category. [Scott]
...
sipmle fields that often need emphasis added or links. It is not currently
...
docment, or template is actually on a desk and in a workflow). [David]
...
The element admin profile now automatically adds the currently selected site
context to new elements, thus generally saving a step when creating new
elements. [JoE<atilde>o Pedro]
...
Images with no dimension greather than the C<THUMBNAIL_SIZE> F<bricolage.conf>
directive are no longer expanded to have one side at least C<THUMBNAIL_SIZE>
pixels, but are left alone. [David]

=item *

Thumbnails are now displayed when searching media to related to an element.
[David]
...
story and media classes. This paramter allows searches on the key name for a
container element that's a subelement of a story or media document. [David]

=item *

Added the following search parameters to the Bric::SOAP::Story C<list_ids()>
method (and therefore the SOAP interface):
...
Added the following search parameters to the Bric::SOAP::Media C<list_ids()>
method (and therefore the SOAP interface):
...
prevent multipple categories with the same names but in different sites from
...
profiles, the maximum length of text and textarea fields is now "0", or
unlimited. [David]
