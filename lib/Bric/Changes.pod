...
Added a C<$INSTALL_VERBOSITY> makefile environment variable that, when set to
...
Added the element_type and output_channel modules to bric_soap. [Scott]

=item *

Added C<ENABLE_GZIP> F<bricolage.conf> directive that turns on compression
of the UI pages through mod_gzip.  This results in pages as much as fifteen
times smaller, with minimal extra server load.  If mod_gzip is found during
installation, the appropriate LoadModule/AddModule lines will be added
to httpd.conf.  Disabled by default. [Marshall Roch; Suggested by Rod Taylor]
...
useful because you can type the first letter in the textbox (or leave it blank
for "All") for the same result.  Alpha search is off by default. [Marshall Roch]
...
for setting a prefix word e.g. 'media' to be prepended to the filename if
