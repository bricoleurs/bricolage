...
contributor type admins create field options with brackets in them. Reported
by Kevin Elliott. [David]
...
Fixed error looking up alert types in the dataase, which affected the alert
...
that element fields that have their maximum length set to 0 can truely be
