...
Added entries to the config settings in ApacheConfig.pm for both Apache 1.3 and Apache 2
to neutralize TRACE requests by returning a forbidden operation response when such  
requests are received. L<http://www.apacheweek.com/issues/03-01-24> [Dave Beaudet]
...
The C<to_preview> option to C<Bric::SOAP::Workflow->publish >> now previews a
document for all output channels, not just the primary output channel. The
