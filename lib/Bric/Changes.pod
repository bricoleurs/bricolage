...
The Story and Media Profiles have been changed to use AJAX to allow fields and
subelements to be added, deleted and reordered on-the-fly, without reloading 
the page.  Elements are displayed recursively, so a story's entire contents can
be edited from one screen, as opposed to one screen per subelement as it was
previously.  Elements are reordered via drag-and-drop, instead of the previous 
column of numbered select lists.

=item *

The Source Profile can now create a new organization on-the-fly.

=item *

Error details (including oft-requested stack traces) are now viewable directly
from the 500 error page, instead of having to check the error_log or view the
error page's source.  To avoid scaring lay people, the details are only shown
(via Javascript) when the new "View Error Details" link is clicked.
...
Codeselect field Perl code can now return an array of array references or a
hash reference, in addition to the previous support for an even-sized array
reference. [David]
...
Added tests for C<Bric::SOAP::Auth>. [Scott]
...
objects found". It was supposed to have been fixed in 1.10.1, but that fix
unfortunately only fixed stories. Reported by Tom Kjeldsen. [David]
...
Fixed highlighting when sorting by Element on desks (bug 1185). Patch from
Brad Harder.
...
respectively, on desks and on My Workspace, works again. Reported by Brad
Harder. [David]
...
Fixed autopublishing related media and stories so that multiple level relates
...
Moved translation-specific Javascript and CSS files into "i18n/" folders under
their respective folders in comp/media/. [Marshall]
...
an asset belongs to if there are multiple sites. [Marshall]

=item *

Fixed appearance of template diffs.  Insertions and deletions now have color
and no longer show underlines and strikethroughs. (bug 1171) [Marshall]
...
Fixed a warning from bric_republish when run without a password.  Thanks to
John Gillis for the spot. (bug 1188) [Marshall]

=item *

Fixed the "Site" dropdown in searches to only require READ permissions
rather than EDIT. Spot and quasi-patch by Chris Sutton.
