...
The installer now requires PostgreSQL 7.3 or later. This is becauase 7.2
...
deleting any source, element, site, worflow, or other related object that has
...
to the mysterious but very rare disappearence of stories from Bricolage.
[David]
...
that time, rather than immediate burning the document and then scheduing the
...
Make slug actually be required when ALLOW_SLUGLESS_NONFIXED isn't enabled,
...
The Burner object's C<encoding> attribute is now settable as well as readable.
[David]
...
C<list_ids()> methods of thestory, media, and template (formatting) classes
...
(C<--published-only> fof F<bric_republish>) now correctly republishes the
