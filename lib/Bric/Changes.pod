...
connection caching. This change somehow eliminated problems running Bricolage
on Apache 1.3.28. It's also the right thing to do. This does bump up the
minimum required version of DBI to 1.18, though the latest version is always
recommended. Thanks toe Perrin Harkins for the suggestion. [David]
...
Vastly improved the speed of the query that lists events. [Mark]
...
Thes tended only to confuse users when they were just starting to use
...
into Bricolage. The remaninging authentication error log messages are:
...
0 is the Adminstrator's user_id this can cause problems. [Sam]
