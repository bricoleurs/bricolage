...
Added support for MySQL, from Andrei Arsu's Google Summer of Code 2006 project.
...
Added support for Apache 2. Updating to Apache 2 from a Bricolage install
using Apache 1.3 is not currently supported. [Scott, Chris Heiland]
...
had to choose an action from a select list and scroll all the way to the bottom
to execute it, you can now check in, move, check out or delete an asset in
one click, without a full page refresh.
...
to true I<before> it is published, instead of aftewards. This both prevents an
exception when a template tries to save a story that has not been published
...
C<template_instance> tables that improperly has C<ON DELETE SET NULL> clauses
...
directory, instead over replacing th existing F<Pg.sql>. [David]
...
Story and media subelements are once again proparly deleted from within their
profiles when the "Delete this Profile" checkbox is checked and either the
"Save" or the "Save and Stay" is pressed. Reported by Chris Schults (Ticket #
1240). [David]

=item *

Searches for media to relate to a document that search by URI now properly
...
`make dev` no longer supports the "single" installation mode, since it has
