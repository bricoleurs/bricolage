...
been integrated into C<make devtest>, yet. [Scott]

=item *

The number of items on a desk can now be displayed next to the desk's name in
the navigation menu, e.g. "Edit (10)". It is configurable on a per-user basis
via the new "Show Desk Asset Counts" preference. Based heavily on a patch from
Alex Howarth. [Marshall]
...
and an outdated warning about Apache::SizeLimit. [Marshall]
...
when auto publishing related assets. If the automatic publication of a related
story or media asset will not be possible then the publish request will fail.
It can be changed to "warn", the previous behaviour, if required. [Paul
Orrock]
...
have a trailing slash, as was the case in Bricolge 1.8 and earlier. [David]

=item *

Aliases are once again identifyable on desks by a different background color
...
contributors. They are now properly sorted by order. Reported by Bret Dawson.
[David]

=item *

Documents without cover dates will no longer have broken URIs. Reported by
Christian Niles. [David]

=item *

The Help popup button works again in Interenet Explorer. Reported by Chris
Heiland. [David]
...
List of templates deployed is now reported properly after a successful deploy
[Paul Orrock]
