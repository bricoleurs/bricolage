...
upgrade script, instead. See F<inst/upgrade/1.7.0/delete_old_componnets.pl>
for an example. [David]

=item *

Bricolage now runs under a DSO C<mod_perl> as long as it uses a Perl compiled
with C<usemymalloc='n'> I<or> C<-Ubincompat5005>. See
...
creating stories of the wrong type. [David]
...
Story", "Find Media", and "Find Template" when there is more than one site and
the "Filter by Site Context" peference is not turned on. [David]
...
classes no longer return C<undef> in a scalar context when there are not
...
In F<lib/Bric/Biz/AssetType/Parts/Data.pm> constant C<ORD>, changed 'name' to
'key_name' (name was replaced with key_name throughout that module, by the
way...). [Scott]
...
Template Toolkit burners to be available again. [David]

=item *

Misspelling the name of a key name tag in Super Bulk Edit no longer causes all
of the changes in that screen to be lost. [David]
