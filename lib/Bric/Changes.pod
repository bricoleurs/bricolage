...
=head1 VERSION 1.11.0 "Summer of Code"
...
Input channels.  Expand me. [Marshall]
...
New "formatting_version," "media_version," and "story_version" tables have been
added, which replace some of the columns in the "(story|media|formatting)_instance"
tables, to allow for multiple instances per version.  To this end, some references
to the *_instance tables have been changed:

=over 8

=item *

The C<version_id> parameter to Bric::Biz::Asset::Business::Media and
Bric::Biz::Asset::Business::Story's C<list()> and C<lookup()> have been changed 
to C<instance_id>.  C<version_id> now refers to a specific version of an asset,
which is no longer the same as an instance.

=item *

The C<story_instance_id> and C<media_instance_id> parameters to Bric::Biz::OutputChannel
are now C<story_version_id> and C<media_version_id>.  There is no replacement for
instances, since ICs and OCs are only associated with versions.
...
=head1 VERSION 1.9.0 "Back to the Future" (change that if you want :)
...
filename of media documents to be set to a unique, automatically generated
filename on upload, (off by default). Also added the C<MEDIA_FILENAME_PREFIX>
directive for setting a prefix word, such as "media", to be prepended to the
filename if desired (off by default). [Paul Orrock / Digital Craftsmen]
...
backward compatability, members of the "All Users" group (that is to say, all
...
plaintext password in the shell history. [Marshall]
...
When started up by the super user, F<bric_queued> now runs as the same user as
...
warning now, instead. Reported by Bill Cappell. [David]

=item *

Bric::Biz::Workflow now caches calls to C<allowed_desks()>. This will allow
desks to render *much* faster, since most assets on a desk will list the same
...
publiished will be republished, and only the last published version will be
republished, rather than any versions created since the last publish.
Suggested by Serge Sozonoff. [David]

=item *

Added the C<secret> paramter to the C<list()>, C<list_ids()>, and C<href()>
method of Bric::Util::Grp. [David]
...
in the search results when attemting to relate a media document to an element.
...
Changed the C<published_version> parameter to the list() method
...
Fixed bug 975 where media uploads were failing with an SQL error due to a
typo. Patch from Rod Taylor.
...
cause the bizarre "Could not find httpd.xml" error. Reported by Curtis
Poe. [David]
...
in one site but not the target site of the upload. [David]
...
When a catgory is added to a story that creates a URI conflict, the new
category is does not remain associated with the story in the story profile
...
All calls to C<lookup()> methods by the SOAP that pass string values to look
up (such as category URIs and site, workflow, and desk names) are SQL escaped
so as to prevent conflicts similar to those noted above for usernames.
...
C<ANY()> now works properly for the output_channel_id parameter to the list()
method of the story and media classes. Reported by Serge Sozonoff. [David]
...
diretive is enabled. Reported by Scott. [David]

=item *

When a template is checked out of the library and then the checkout is
cancelled, it is no longer left on the desk it was moed into upon the
...
Eliminated compile-time warnings from
