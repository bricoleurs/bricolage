...
checked in and deployed a template upon upload. Teamplates can still be
...
Added C<ANY> fuction to be used with select parameters to story, media, and
...
above the textarea. This feature is enabled when the Max size atribute for the
...
everytime someone adds another character beyond the maximum. [Paul
Orrock/Digital Craftsmen]

=item *

Added a feature to display image thumbnails in the search results for media
objects that are image objects. This feature uses the Perl module Imager and
the relevant image library for each format you want to display. It can be
turned on or off using the C<USE_THUMBNAILS> F<bricolage.conf> directive. See
...
of fetching data from the database. Suggested by Josh Berkus. [David]
...
mesages and JavaScript messages. [Kang-min Liu]
...
value of the C<VHOST_SERVER_NAME> directive.
...
Filter by site context now works. [Joao Pedro]

=item *

Several Alert Type fixes. Rule regexp matching (=~, !~) now handles
patterns containing slashes (important for URIs, for example).
Attributes no longer show up as stringified hash
references in subject or message variable substitution.
$trig_password was removed from the Profile as it caused
an error and was useless anyway. [Scott]

=item *

Fixed Template Element list, where container elements appeared twice. [Joao
Pedro]
...
The creation of the Bricolage PostgreSQL user and database during installtion
...
URL if it is not alreaded appended. Reported by Marshall Roch. [David]
...
Fixed a bug in `bric_soap story create/update` caused by refactoring in
version 1.7.0. Found by David during a demo. [Scott]

=item *

Fixed typo of CHECK_FREQUENCY in Bric::Config that would've made it
always use the default of 1, since version 1.43 (2002/05). [Scott]
...
instance of Bricolage, these new features allows the Bricolage administrator
...
Bricolage once again works with Perl 5.6.1 (and all of its lovely bugs).
[David]
