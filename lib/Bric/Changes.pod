...
references to button images with text on them and strings fetched from the
database. [ClE<aacute>udio Valente]
...
Added preview link to every element profile of a story profile. [David]
...
Bric::Biz::Category->get_children now properly returns the children for the
root category. Thanks to Sam for the spot! [David]
