...
New help pages for the desstination, server, and action profiles. [Geoff
Richards]
...
The media simple search now searched by URI again. [Mark]

=item *

Now HTML-escaping the contents of the context content displayed for
subelements so as to prevent HTML in fields from messing up thedisplay. [Mark]
...
missing Desination associations so that the user can tell which output
...
than that defined by its documtent type element, preview will now correctly
...
Adding Extentions to a Bric::Util::MediaType object when creating it via that
