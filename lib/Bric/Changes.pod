...
Bric::Biz::Asset::Business::Parts::Tile::Container to take a list of key name
arguments, and to return only the container subelements with those
keynames. This is most useful in templates, where it's fairly common to get a
...
When the SOAP server serializes element templates, it now correctly identifies
the element by its key name, rather than its name. Thanks to JoE<atilde>o
