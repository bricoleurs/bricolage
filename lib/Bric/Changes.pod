...
Added the C<RELATED_MEDIA_PROFILE> F<bricolage.conf> directive to redirect
users into the new media profile when creating a new media document from a
related media element, which is only allowed when the C<RELATED_MEDIA_UPLOAD>
directive is enabled. This is useful when editors what to be able to edit the
newly uploaded media document and then return directly to the related media
element from which they created it. [David]
