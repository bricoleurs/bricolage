...
It is now much harder to create media type or story type elements without site
and output channel associations. This should eliminate errors when users try
to create documents based on types without output channel associations.
[David]
...
Made the C<bric_media_upload> contrib script work with the 1.7.0 DTD.
Also fixed a bug in its use of File::Find and added a couple options.
