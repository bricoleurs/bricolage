...
When creating a related media, if clicking the "Create" button without both
selecting an element type and seleting a file to upload now returns a message
to the user suggesting that she do so. [Nick Legg]

=item *

Fixed a number of issues related to UI behavior in the event of a publish
failure. A publish failure from a desk, workspace, or document profile now
behaves consistently and properly respects the
C<PUBLISH_RELATED_FAIL_BEHAVIOR> F<bricolage.conf> directive. Furthermore,
failed publishes no longer result in a document being checked in when it was
checked out. Instead, the document will look just like it did before the
publish failed. The only difference is that the version will increment. This
is somewhat annoying, but far better than what we had before, and most folks
likely won't notice the change. Thanks to Nick Legg for the report and Denison
University for sponsoring this work (Bug #277). [David]

=item *

The select list for output channel includes in the output channel profile now
includes the names of the sites each output channel is part of (Bug # 279).
[Zdravko Balorda]

=item *

Relative F<http.conf> directives are now properly processed. [Florian Ragwitz]
...
The C<find_or_create_alternate()> method of
L<Bric::Biz::Asset::Business::Media::Image> now properly respects the URI case
settings of the output channel when searching for an existing alternate image.
Thanks to Bret Dawson for the catch and the fix. [David]

=item *

Fixed the C<inactive> parameter to the Template, Story, and Media C<list()>
methods so that it returns the appropriate records, instead of always
returning no records. [Greg Heo].

=item *

Commas no longer appear escaped with a backslash in the breadcrumb. Thanks to
Michael Herring for the report (Bug #245). [Greg Heo]

=item *

The C<find_or_create_alternate()> method of
L<Bric::Biz::Asset::Business::Media|Bric::Biz::Asset::Business::Media> now
uses the correct output channel and respects the URI format (Bug #274). [Bret
Dawson]

=item *

Searching for assets by category (in simple search) will no longer return
items that used to be in that category in previous versions (Bug #242). [Greg
Heo]

=item *

Fixed bug where, after opening the media search options in advanced mode, one
could not return to a simple search. Thanks to Michael Herring for the report
(L<Bug #282|http://bricolage.lighthouseapp.com/projects/29601-bricolage/tickets/282>).
[Nick Legg]

=item *

The "Return" button now works when viewing an asset in IE or Firefox 4+.
Thanks to Michael Herring for the report
(L<Bug #240|http://bricolage.lighthouseapp.com/projects/29601-bricolage/tickets/240>).
[David]

=item *

Deleting an output channel or category from a story or media document no
longer causes the Notes, Trail, Bulk Edit, and Diff/View buttons to stop
working. Thanks to Michael Herring for the report
(L<Bug #281|http://bricolage.lighthouseapp.com/projects/29601-bricolage/tickets/281>).
[David]

=item *

Drag and drop of elements now works on IE 9 Thanks to Nick Legg for the report
(L<Bug #250|http://bricolage.lighthouseapp.com/projects/29601-bricolage/tickets/250>).
[David]

=item *

Eliminated "Use of qw(...) as parentheses is deprecated" warnings on Perl
5.14. [Florian Ragwitz]

=item *

Fix for the possibility that a FTP server would return a list, not a text message, per http://www.gossamer-threads.com/lists/bricolage/users/40893#40893 [Phillip Smith]

=back

=head1 Version 2.0.1 (2011-02-09)
...
proxy server that's handling SSL mappings. Thanks to Alex Krohn for the
report. [David]
...
Added the C<asset_counts()> method to L<Bric::Biz::Workflow::Parts::Desk> and
put it to use providing the desk counts in the side navigation. This provides
a huge performance improvement over the previous method of showing counts,
which was to load every asset on every desk on every load. Didn't scale well
for desks with lots of assets on them (Bug #239). [David]
