...
Added "Input Channels."  Changes:

=over

=item Versions and instances are no longer synonymous.  Each version of a story
now contains an instance for each input channel.  API calls that used to refer
to "instance_id" are most likely now "version_id," including but not limited to:

=over

=item Bric::Util::Job::list(story_instance_id => ?)
=item Bric::Util::Job::list(media_instance_id => ?)
...
Moved burner association from element types to output channels. Now a single
output channel must have all of its templates use the same burner. [David]
...
FORMAT, an SCALAR. Use of imported functions in PHP templates requires
