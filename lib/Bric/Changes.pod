...
Changed code to show the first displayable data field of a sub-element when
...
The verson 1.8.8 upgrade script that moves media files around no longer fails
when the media directory doesn't exist (becase no media documents have been
...
Backported database upgrade permissions changes from Bricolag 1.10 so that
they permissions are set for the super user for upgrade scripts such as
...
Fixed file names for files uploaded by MSIE on Windows. Reported by Wayne
Slavin. [David]
...
'Default Asset Sort' preference, or title if it is not set. [Paul Orrock]
...
display a message indicating that the related document cannot be published
...
rather than names. Template sorting issues reported by Scot. [David]
