...
"QUIET", eliminates many of the questions asked during installation, falling
back on the default values. [Rod Taylor]
...
media documents to alias and the C<USE_THUMBNAILS> F<bricolage.conf> diretive
...
The SOAP interface for importing and exporting elements now properlly has
"key_name" XML elements instead of "name" XML elements. The changes are
backwards compatable with XML exported from Bricolage 1.8.0 servers, however.
...
types of objects. Also added the ability manage group membship within the
...
Bulk Publish now ensures that users have at least READ access to catgories
...
of the user or contributor oject are now properly saved. [David]

=item *

When cloning a story, the list of output channels to select from is now
limited to the site the current workflow is in. Thanks to Serge Sonzonoff for
the spot! [David]
