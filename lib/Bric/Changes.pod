...
installations, which are necessary for packagers. [Masrhall]
...
F<httpd.conf>. Disabled by default. [Marshall; Suggested by Rod Taylor]
...
for the same results. Alpha search is off by default. [Marshall]
...
The URI formats are now more flexible. You can now use a lot more parts of the
...
Added support for multiple WYSIWYG editors.  C<ENABLE_HTMLAREA> has been 
replaced by C<ENABLE_WYSIWYG> and C<WYSIWYG_EDITOR>.  You can now choose between
htmlArea 3 and the new Xinha, a more active fork of htmlArea (recommended).
More editors possibly coming soon. See Bric::Admin for details. [Marshall]
