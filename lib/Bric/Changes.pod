...
Bric::SOAP::Media are fully implemented and tested.  The command-line
client - bric_soap - is complete. [Sam]
...
The slug is stored in the database for both fixed and non-fixed
stories. This is in anticipation of adding the ability to use the slug
for fixed story URIs later on. Thanks to Darren Graves for the patch.
...
Fixed broken "Preview" button in Read-only story profile. [Matt Vella]
