...
means a large saving in download size of cloned production systems.
...
in bracket group," which appeard in searches and on desks. Thanks to Guido
...
MEDIA_UNIQUE_FILENAME is now fixed so that the filename creation comes after
the id generation and therefore correctly saves the renamed file.[Paul Orrock]
...
Fixed Bric::App::Util::last_page so that if the last page was a preview request
we get the correct UI page back by jumping back another two pages. [Paul 
Orrock]
