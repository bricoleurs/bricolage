...
Eliminated letter graphices for "Published" and "Deployed" labels in story,
...
Refactored the left nav to perform better and be easier to maintain. It now
also sorts the admin menu items in alphabetical order by the localized name of
each. [David]
...
Eliminated the old "P" and "D" graphics indicating the publish and deploy
status of documents and templates in the search interface, replacing them with
actual letters. [David]

=item *

The Element Type profile is now much smarter about displaying sites form
association with an element type. If the user has access to only one site and
