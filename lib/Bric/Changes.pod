...
Revised Bric::Biz::Keyword implementation to improve performance. The
new implementation no longer relies on the Bric::Util::Grp system.
Also implemented API and database support for Media keywords.  [Sam]

=item *

Added support for search Media by category and file name through the
SOAP inerface and standard API. [Sam]

=item *

Added new --save-cookie-file and --use-cookie-file options to
...
timeouts on large workloads.  Currently only supports the "worflow
publish" command but it will be expanded to other long-running
commands in the future.  [Sam]
...
Fixed bug that was preventing 'make upgrade' from running database
upgrade scripts. [Sam]
