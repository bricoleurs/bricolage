...
expired and unexpired stories and media. [David]

=item *

If a story is passed to the C<burn_another()> or C<preview_another()> burner
methods that is the same as the story currently being burned, it will no
longer be published or previewed again by C<burn_another()> or
C<preview_another()>, thus mitigating the possibility of infinite loops.
[David]
...
Having a custom field beginning with a leading underscore was causing
an error in comp/widgets/wrappers/sharky/header.mc. [Scott]

=item *

Errors thrown when setting element field values will not be displayed as error
messages within the context of the element profile, rather than throwing a
fatal exception. This is useful for invalid dates, foe example. [David]
...
Fixed database DDLs to work with PostgreSQL 8.1. [David]
...
New elements and fields will now have any leading or trailing whitespace
removed, rather than converted to underscores. Reported by Scott Lanning.
[David]
...
In a story or media profile, eleting a subelement with the same ID as a field
