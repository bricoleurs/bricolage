...
ContribType and Destinations SOAP modules. [Scott]
...
check for %{categories} in an Output Channel URI format. This directive should
be used with caution since this is not the recommended behaviour. [Paul
Orrock]
...
Created two new make targets, "install_files" and "install_db", to allow 
the files to be installed without doing anything with the database.  Useful 
for packagers who need to let the user create the database on her own.  
"make install_files install_db" is equivalent to "make install". [Marshall]
