...
directive with speficic reference to SFTP versions. [Alex Krohn]
...
for example, L</foo/bar> with L</foo_bar>. Reported by Scott (Bug # 1368).
[David]
...
Updated the minimum required version of DBD::Pg to 2.0.0, since the
three-value version numbers adopted with 2.0.0 no longer correctly compare to
the old two-value version numbers, such as the old requirement for version
1.22. The result was that the installer thought that 2.0.0 and higher were
older than 1.22 and always tried to install them. [David]
