...
Added entries to the config settings in ApacheConfig.pm for Apache 1.3 
to neutralize the potentially malicious use of HTTP TRACE requests by returning 
a forbidden operation response when such requests are received. 
