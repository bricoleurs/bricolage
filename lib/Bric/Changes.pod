...
Adding notes to image, video, or audio media documents no longer causes na
...
forcing a second redirect in such cases. Reportec by Marshall Roch. [David]
...
they can all be used in a C<< <%once> >> block like this:
  ...
  unless ($burner->get_mode == SYNTAX_MODE) {
      # Do stuff with $story or $template.
  }
  </%once>
