...
uploading and saving new image files. Also documented extra methods not
previously documented in L<Bric::App::MediaFunc|Bric::App::MediaFunc>.
Reported by Michael Holden. [David]
...
type of a file, thus more likely supporting files that it might have been able
to support before. [Rod Taylor]

=item *

F<bric_republish> no longer reverses the meaning of the C<--published-only>
option. [David]

=item *

Publishing via SOAP works again. It was broken in 1.8.9, where it would appear
to succeed, but no story or media would ever actually be distributed to the
content server. [David]

=item *

Updated the Bricolage users list email address and archive link in
Bric::Admin/INSTALL. Reported by Michael Cummings [David]
...
Silenced status messages when previewing media document under the
...
The Jobs manager now sorts pending jobs by scheduled time rather than date.
[David]
...
longer have to select the role if the contributor has only one role. Reported
