...
Eliminated encoding errors when using the SOAP interface to output stories or
media with wide characters. Reported by Scott Lanning. [David]
...
Reverting (stories, media, templates) no longer gives an error. Also fixed
one or two other similar problems, apparently due to changing some columns
from integer to boolean. [Scott]
