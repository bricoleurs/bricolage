...
Fixed control.html so that it pops the last two pages off the stack. Now we get 
the correct UI page if we use a return button in the UI and the last page was 
a preview request. [Paul Orrock]
