...
In the Element Type (new name) Profile, when adding a new Radio Buttons,
Pulldown, or Select custom field, you can now escape commas (that otherwise
separates the Options from their Label) with a backslash so that it will
appear literally in either the value or label. [Scott]
