...
deleted stories, media, and templates. Also added the 'Subelement' dropdown,
which allows searching on Stories by subelement. [Scott]
...
New "Code Select" custom field allows you to add arbitrary Perl code to
...
Bric::Biz::AssetType::Data object in Bric::Biz::Asset::Parts::Tile::Data from
C<element_data_id> and C<get_element_data()> to C<field_type_id> and
C<get_field_type()>. [David]
...
element type and field type attributes and are easily referenced in the
element type and field type tables. This change significantly reduces the size
of the database and has the potential to increase performance when getting and
setting field values, especially in templates. [David]

=item *

Major refactoring of the field types class, Bric::Biz::AssetType::Parts::Data.
...
Added much more fine-graine field type management to the Element Type SOAP
interface. More field attributes can now be updated via SOAP. [David]
