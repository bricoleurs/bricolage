...
exception when a template to save a story that has not been published before,
and eliminates the need for templates to seet the C<publish_status> on a story
before calling C<publish_another()> for a another story that will search for
the current story in the database. Reported by Rod Taylor (Ticket # 1230).
[David]
