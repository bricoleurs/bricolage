...
ContribType, Destination, and Preference SOAP modules. [Scott]

=item *

When a page finishes loading, the first text or textarea field in the form now
automatically gets the focus. [David]

=item *

Added upload and download feature to the template interface, so that templates
can optionaly be uploaded or downloaded. Suggested by Chris Sutton. [David]
...
Removed call to removed component F<comp/widgets/qa/qa.mc> from the side
...
Fixed a foreign key constraint on "media_element.related_story__id" that was
pointing to media.id instead of story.id. Reported by Rod Taylor. [David]

=item *

Added a 'make devclone' option which does the same as 'make clone' but removes
all of the locally previewed files, the obj files, staging and sandboxes. This
means a large saving in download size of cloned production systems. [Paul
Orrock]

=item *

Added C<ALLOW_URIS_WITHOUT_CATEGORIES> F<bricolage.conf> directive to turn off
check for "%{categories}" in an output channel URI format. This directive
should be used with caution since this is not the recommended behaviour. [Paul
Orrock]

=item *

Added tooltip to show the underlying Bricolage database ID when the UUID is
hovered over with the mouse. This is the partner to showing the UUID when the
hovering over the Bricolage ID in a desk view. [Paul Orrock]
...
Changed the database coluns for the "name", "key_name", and "description"
attributes of
...
packagers who need to let the user create the database on her own. "make
install_files install_db" is equivalent to "make install". [Marshall]
...
contextd of Bricolage 1.10. Reported by Ryan O'Toole. [David]

=item *

The View/Edit link has been restored to storis, media, and templates on desks,
...
Fixes for UI display in Firefox and IE. Particularly Meta Assets displaying
down the page in IE, stretching out the banner in desks, corrected side menu
position in IE and removed reference to broken images left in from CSS
reworking. Also fixed textarea which was ignoring cols setting in IE and set
textarea col width for bulk edit to 80 wide, rather than 30. [Paul Orrock]
...
Disabled the "sticky" scrollbar in the story profile. It just didn't work too
well. [David]
...
Fixed the size of the preview links on the desktop. [David]
...
When multiple versions of a document are published before any of the publish
jobs for those versions is executed no longer leads to publishing errors when
the older version is published. Thanks to Rod Taylor for the spot! [David]

=item *

The element trace generated for burn errors (those created by C<<
$burner->throw_error() >>) no use the place attribute of the element where the
error was thrown, rather than the object order, since the place corresponds to
the "Position" select list orderings in the UI, and therefore makes it much
simpler to figure out where the error is. [David]
