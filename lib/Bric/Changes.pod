...
Added C<make dev> makefile target for installing a development version
straight out of a Subversion checkout. It performs a standard installation,
then symlinks F<bin/>, F<comp/> and F<lib/> back to the Subversion checkout.
[Marshall]
...
There is now a "Preserve Formatting" option (disabled by default) in Bulk Edit
...
story title. So, for example, you can now use C<< <tmpl_var uri> >> or
C<< <tmpl_var cover_date> >> without creating a F<.pl> template to set them
up. [David]

=item *

Refactored the separate C<_add_resource()> methods in each of the burner
subclasses into Bric::Util::Burner itself as C<add_resource()>. [David]

=item *

Arguments can now be passed to the C<display_element()> methof of the Template
Toolkit burner and they will be set up as variables in the stash of the
execution context of the element template. [David]
...
The old C<link> variable has been deprecated, since it cannot handle an
element that has both a related media and a related story. [David]
