...
selected for the Story Type or Media type (respecively), so as to prevent
the inadvertant creation of documents of the wrong type. [David]
...
Added C<bric_media_dump> script to C<contrib/>. [David]
...
and the "Filter by Site Context" peference is not turned on. [David]
...
C<--locale=C> so as to avoid collation issues with non-Western characters. See
...
Added output the the Apache log consisting of the 20 most recent events on an
...
STDERR when there is an error, instead of just printing it to the system log.
[David]

=item *

Existing jobs are now executed within their own transactions, as opposed to no
transaction specification. This means that each must succeed or fail
independent of any other jobs. New jobs are executed before being inserted
into the database so as to keep them atomic within their surrouding
transaction (generally a UI request). All this means that transactionality is
much more intelligent for jobs and will hopefully eliminate job deadlocks.
Reported by Mark Jaroski, Serge Sozonoff, Patrick Michael Kane, and others.
[David]

=item *

The workflow, destination, and output channel profiles only display site
information if the user had EDIT access to more than one site. Suggested by
Scott Lanning. [David]
...
parameters to the story and media class <list()> methods that were supported
but previously undocumented. [David]

=item *

Variables are now properly decoded to UTF-8 when they are populated with
data from the database. [David]
...
that any templates that convert content to other character sets must change
the way they do so. For example, templates that had used C<< <%filter> >>
blocks to convert content to another encoding using something like
C<Encode::from_to($_, 'utf-8', $encoding)> must now use something like
C<$_ = Encode::encode($encoding, $_)>, instead. [David]

=item *

Added C<encoding> attribute to Bric::Util::Burner so that, if templates are
outputting something other than Perl utf8 decoded data, they can specify what
they're outputting, and the file opened for output will from the templates
will be set to the proper mode. Applies to Perl 5.8.0 only. [David]
...
Users can no longer implicitly publish related stories and media that they
don't have PUBLISH access to. [David]

=item *

Users can no longer publish realted documents that are in workflow but not on
a publish desk. [David]
...
objects to be returned. It returns an empty array reference, instead. [David]
...
C<make clone> once again properly copies the F<lib/Makefile.PL>
...
metdata forthe the C<key_name> attribute instead of an C<undef> for the
...
Creating a story, checking it in, checking it out, and then cancelling the
checkout no longer incorectly removes the story from workflow. Reported by .
[David]
...
include a slash ("/"). Reported by Kevin Elliott. [David]

=item *

An attempt to create a new template with the proper extension as part of the
name (e.g., "foo.mc" fora Mason template) now keeps the extension instead of
changing the dot to an underscore and adding the extension (e.g., it stays
"foo.mc" instead of becoming "foo_mc.mc"). This is especially useful when
...
Content submitted to Bricoalge in the UTF-8 character set is now always has
the utf8 flag set on the Perl strings that store it. This allows fields that
have a maximum length to be truncated to that length in characters instead of
bytes. Thanks to Kang-min Liu for the spot! [David]
...
them before inserting them, as doing so can truncate in the middle of
multibyte characters and lead to 'invalid byte sequence for encoding
"UNICODE"' database errors. [David]
...
an C<ANY> argument to these parameters could cause a well-populated database
to lock up with an impossible query for hours at a time. [David]
