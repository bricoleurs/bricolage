...
Alert Type rule regexp matching (=~, !~) now handles patterns
containing slashes (important for URIs, for example). [Scott]

=item *

Alert Type attributes no longer show up as stringified hash
references in subject or message variable substitution. [Scott]
