...
Bric::BC::Org - Bricolage Interface to Organizations
...
=head1 SYNOPSIS
  ...
  # Constructors.
  my $org = Bric::BC::Org->new;
  my $org = Bric::BC::Org->lookup({ id => $id });
  my @orgs = Bric::BC::Org->list($search_href);

  # Class Methods.
  my @org_ids = Bric::BC::Org->list_ids($search_href);
...
companies for whom a person represented by a Bric::BC::Person object works, or
...
The primary use for Bric::BC::Org as of this writing, however, is to associate
people (Bric::BC::Person objects) with companies and their addresses. These
associations are created by the Bric::BC::Org add_object() method, and by the
Bric::BC::Org::Person subclass it returns. See Bric::BC::Org::Person for its
additions to the Bric::BC::Org API.
...
=item $org = Bric::BC::Org->new

=item my $org = Bric::BC::Org->new($init)

Instantiates a Bric::BC::Org object. A hashref of initial values may be passed. The
...
=item my $org = Bric::BC::Org->lookup({ id => $id })

Looks up and instantiates a new Bric::BC::Org object based on the Bric::BC::Org
...
Too many Bric::BC::Org objects found.
...
sub lookup {
    ...
    die Bric::Util::Fault::Exception::DP->new({
      msg => 'Too many Bric::BC::Org objects found.' }) if @$org > 1;
    return @$org ? $org->[0] : undef;
}

################################################################################

=item my (@orgs || $orgs_aref) = Bric::BC::Org->list($params)
...
B<Side Effects:> Populates each Bric::BC::Org object with data from the database
before returning them all.

B<Notes:> This method is overridden by the list() method of the
Bric::BC::Org::Person class.
...
=item my (@org_ids || $org_ids_aref) = Bric::BC::Org->list_ids($params)

Functionally identical to list(), but returns Bric::BC::Org object IDs rather than
...
Returns the ID of the Bric::BC::Org object.
...
B<Notes:> If the Bric::BC::Org object has been instantiated via the new()
...
Returns the common name for the Bric::BC::Org object.
...
Sets the common name of the Bric::BC::Org object. Returns $self on success and undef
on failure.
...
Returns the formal name for the Bric::BC::Org object.
...
Sets the formal name for the Bric::BC::Org object. Returns $self on success And Undef
on failure
...
Activates the Bric::BC::Org object. Call $org->save to make the change persistent.
Bric::BC::Org objects instantiated by new() are active by default.
...
Deactivates (deletes) the Bric::BC::Org object. Call $org->save to make the change
persistent.
...
Returns $self if the Bric::BC::Org object is active, and undef if it is not.
...
Returns $self if the Bric::BC::Org object is personal, and undef if it is not. By
...
Associates a Bricolage object with the Bric::BC::Org object, returning the relevant
subclassed Bric::BC::Org object. See the Bric::BC::Org::* subclasses for the
relevant methods for associating specific Bric::BC::Org::Parts::Addresses with
...
B<Side Effects:> Returns a subclassed Bric::BC::Org object with methods for
managing the association with the Bricolage object. Currently, only Bric::BC::Person
objects may be passed, thus returning Bric::BC::Org::Person objects.
...
Returns a list of Bric::BC::Org::Parts::Address objects. Returns an empty list when
there are no addresses associated with this object, and undef upon failure. See
the Bric::BC::Org::Parts::Address documentation for its API.
...
B<Side Effects:> Stores the list of Bric::BC::Org::Parts::Address objects
internally in the Bric::BC::Org object the first time it or any other address
method is called on a given Bric::BC::Org instance.

B<Notes:> Changes made to Bric::BC::Org::Parts::Address objects retreived from
this method can be persistently saved to the database only by calling the
Bric::BC::Org object's save() method.
...
Adds and returns a new Bric::BC::Org::Parts::Address object associated with the
Bric::BC::Org object. Returns undef on failure. See the Bric::BC::Org::Parts::Address
...
persistently saved to the database only by calling the Bric::BC::Org object's
save() method.
...
If called with no arguments, deletes all Bric::BC::Org::Parts::Address objects
associated with the Bric::BC::Org object. Pass Bric::BC::Org::Parts::Address object
IDs to delete only those Bric::BC::Org::Parts::Address objects.
...
B<Side Effects:> Deletes the Bric::BC::Org::Parts::Address objects from the
Bric::BC::Org object's internal structure, but retains a list of the IDs. These
will be used to delete the Bric::BC::Org::Parts::Address objects from the database
when $org->save is called, then are deleted from the Bric::BC::Org object's
internal structure. The Bric::BC::Org::Parts::Address objects will not actually be
deleted from the database until $org->save is called.

B<Notes:> If called with a list of Bric::BC::Org::Parts::Address object IDs,
del_address() will only delete those address object if they're associated with
the current Bric::BC::Org object.
...
Saves any changes to the Bric::BC::Org object, including changes to associated address
(Bric::BC::Org::Parts::Address) objects. Returns $self on success and undef on
failure.
...
B<Side Effects:> Cleans out internal cache of Bric::BC::Org::Parts::Address objects to
