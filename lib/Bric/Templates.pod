...
For a field that can occur more than once in a stor, this method returns its
...
Returns 1 if $eelement is a container element object and 0 if it is a field
element.
...
The burner base class, wher most methods are defined.
...
the burn of the next page, untilall pages specified by the array reference of
...
namespace: C<PUBLISH_MODE>, C<PREVIEW_MODE>, and (for completeness)
...
through wich the story is being burned. Useful when using output channel
...
categoris, as each category association triggers a separate burn.

=item C<< $burner->best_uri($doc) >>

Returns a C<URI|URI> object representing the best URI for the document passed.
Useful for multi-site Bricolage installations, where one story might wich to
...
method simply retruns without doing anything.
...
channel. In publish mode, this method method simply retruns without doing
anything.
...
this story. (Formatting forrelated stories is covered in
...
Again, teh various architectures use the same programming logic and output the
same content (modulo whitespace issues). In each case, the code outputs a
...
=head2 Class Names
    ...
    Conainter Element Type  Bric::Biz::ElementType::Container
