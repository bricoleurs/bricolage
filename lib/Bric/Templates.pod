...
will have. For instance, an 'Editorial' element might define that an Editorial
contains "An author name, a title, and one or more paragraphs". Elements cannot
affect the formatting or look of a story, just the data fields available.

=head2 Templates

For a given element, a template will give a format for the display of the data
fields available in that element. This is where the HTML will come from.
...
element defines the structure of a story. It lists the data fields available to
...
example, maybe the 'column' element above will be used for a column reviewing
music. A music column might want to have a short blurb about the albums they
review. Additionally, say somewhere else on the site, there is another type of
story, "Top Music Picks", that lists blurbs about albums. So we create another
...
topic for the column, a few paragraphs, and a blurb about the album they are
...
data for one element. So let's say we create a new template and associate it with
...
meant to convey a template's relationship to an element.  We'll get into the nuts
and bolts later.
...
to provide further distinction:
...
'Classical' and 'Rock' subcategories will all inherit from the template
associated with the 'Music' category. If there is no need to change how the data
is displayed between any category, you can simply create a template associated
...
presentation of the data for display on one output channel (the web site) versus
...
what categories have templates defined, and how are they used during a publish?
...
detail how the files are laid out. This will hopefully make clear a few things
about templates and publishing.

Let's start with names. Elements are given names when they are created. In our
...
special Perl tags thrown in. So when saving the code of a template to the
file-system, we take the template name and append '.mc' or "Mason element" and
use that as the filename. For our 'column' template, the filename would be
'column.mc'.
...
the output channel's name, its ID is used. Let's just say that the ID for the
...
So, let's assume that we have three column templates: one associated with the root
...
has the opportunity to associate it with one or more categories. The table
...
'$COMP_ROOT/oc_1/column.mc'. It's important to realize that templates do not
...
mason concept of 'dhandler's. While not exactly analogous, it's pretty close.
...
The $num argument is optional. Like the $num argument in 'get_data', it is
