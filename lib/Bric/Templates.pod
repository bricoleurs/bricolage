...
players in the publish process and get the terminology down.  The three 
elements involved in publishing a story are:
...
author then enters business data.  This data is then run through a template
...
those creating a story.  So for example, a 'Column' element might define a 
column as having:
...
Elements can also contain other elements as part of their definition.  For 
for example, maybe the the 'column' element above will be used for a column
...
another type of story, "Top Music Picks", that lists blurbs about albums.  
So we create another another element called 'music_blurb' with these fields:
...
reviewing.  Elements can be nested as deeply as needed.  So an element can
contain an element, that contains another element, etc.

The most important thing to understand about elements is that they simply 
define the set of data that the author will be asked for when creating/editing 
a story.  The element has no bearing on the actual data itself, or the 
...
1 to 1 with an element.  Said a different way, a template knows how to format 
the data for one element.  So lets say we create a new template and associate it
with the 'column' element we defined above.  Maybe we want to format a column 
like this:
...
column, a Theater column and a Gallery column.  We want to collect the same
data for each of these columns, but we want the data to look different when
published to each of these columns.
...
data is displayed between any category, you can  simple create a template 
associated with the root category.  All the categories will inherit from it.


=head2 Templates And Output Channels

There is one final parameter that a template has called an 'Output Channel'.  
For this discussion output channels can be thought of as hidden categories.
...
of these would be considered an output channel, with the web site considered
...
examples above, we created a 'column'.  When you create a template, and choose
an element that it should format, it automatically assumes the name of that
...
Finally the output channel is also used as a  part of the path.  Rather than 
take the output channels name, its ID is used.  Let just say that the ID for 
the primary output channel is 1. So, our column template would live in 
'oc_1/music/rock/column.mc'.

If we assume that our element root is $COMP_ROOT the full path to our 
template is '$COMP_ROOT/oc_1/music/rock/column.mc'
...
with mason might see the similarity between how templates are handled and 
the mason concept of 'dhandler's.  While not an exact analogous, its pretty 
close.
	With a small change to how we create templates, we can get them to 
behave like a Mason 'autohandler'.  In Mason an 'autohandler' is a element
that is called before any other mason element is called.  Moreover, Mason
autohandlers can chain.  For example, assume the following files exist:
...
page.  The easiest way to do this is by using an autohandler.
       If we create a new template in the root category, for the primary 
output channel (for this example, the 'web' output channel with ID 1) then we'll
get the file:
...
Most of this should look pretty normal.  The only difference is the bit of 
perl code in the middle.  There is a call to a method named 'chain_next'.  This 
is where all further content in the chain will go.  So, given this autohandler
...
The content that 'column.mc' outputs after being run will wrapped by the HTML
in the autohandler template.

As a last example of chaining templates, lets say that we wanted some special 
HTML to appear just in the music section, irregardless of what type of story
we are publishing.  We would then create a new template that was not associated
with an element, but was associated with the 'Music' category and the primary
output channel:
...
Writing template code is no different than writing Mason code.  If you've 
written Mason code, you should have no problem writing templates.  If you have 
not written Mason code before, I suggest you get familiar with it by reading the
HTML::Mason::Devel manpage.  A tutorial on Mason is beyond the scope of this 
document.
...
this convention.  When I speak of an element as defined by 1), I will refer to
it as an Element with a capital 'E'.  After all, its the true "form" of a story;
the definition of a particular type of story.  When I speak of elements as 
defined by 2) and 3) I will use a lowercase 'element'. 
	Furthermore, since a story has two types of elements, lets establish a
way of distiguishing between them.  We'll call all text data fields of a story
'data elements' and the contained fields that are based on an Element, 
'container elements'.  When we don't need to distinguish them, we'll 
collectively refer to them as 'elements'. 
...
=item $story->get_teaser

Retrieve the 'teaser' for this story.  This is basically a short description.

=item $story->get_cover_date

Returns the date this story is tied to (not necessarily when it was published). 
 
A cover date might be 'June 1, 2001' for the June issue of Seventeen magazine, 
but it might actually be published some time before or after that exact date.
...
The current element object contains the real data of a story.  Since a story 
is based upon an Element, the $element object is an instance of that Element.  So
if the Element defined an 'author', 'topic' and 'paragraph' fields you know you
can access those data elements via $element.  Where $element really becomes 
...
The $num argument is optional.  It is meant for data elements that can occur 
more than once, such as 'paragraph's.  If $num is not given the text for the 
first $name data element will be returned.  Note to programmers;  $num starts 
at '1' not '0'.
...
If $num is not given, the first container element object will be returned.  
Note to programmers;  $num starts at '1' not '0'.
...
object.  A short list of methods available for data element objects will be 
...
For container elements that can occur more than once in a story this will 
return this elements place among like container elements.  For example a 
'column' story might contain several 'music_blurb' container elements.  If this 
happens to be the 2nd of 3 'music_blurb' container elements, this method will 
return 2.
...
and some data element objects.  Here is a list of methods you can call on a 
data element object:
...
Here is the code for the 'column' template:
    ...
    <%perl>
        my $n = 1;
        while (my $p = $element->get_container('page', $n++)) {
            $burner->display_element($p);
        }
    </%perl>
...
for the 'page' element.  Note that we skip the 'deck' data element.  This 
element is meant for when this is used in a related story context.  Using 
related stories will be discussed later in another document.
...
of the column, followed by the data for this story.  After the data is output
the 'previous' and 'next' links are displayed.  Note that in the foreach loop
...
A mason concept.  As applied to a Bricolage it is type of variation on a 
template that can form a line of 'chained' templates.
