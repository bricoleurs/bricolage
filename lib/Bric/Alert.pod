...
only those events that follow those rules. Furthmore, in specifying the subject
...
that you will see when you select ot edit an existing Alert Type profile. Here
...
interface (it's deactivated by default - check the "Active" checkbox to activate
it).
...
Uses Perl's <ne> operator internally to determine if the values are not
equivalent.

=item >

Uses Perl's <gt> operator internally to determine if the event value is greater
than the user-entered value.

=item <

Uses Perl's <gt> operator internally to determine if the event value is less
than the user-entered value.
...
variables to custumize the alert on a per-alert basis.

The Alert Type UI "Conent" section features fields for the subject and message,
...
preceed it with a backslash ["\"] and it will be interpreted as a literal dollar
...
not necessary. However, by including variables, you can custommize the message
...
Selecing from the second list of variables will likewise automatically insert
...
have been specified as a recipient of an alert via "Prmiary Email," you will
receive alerts at both addresses. The most common type of contact will likely be
"Primary Email."

You can specify that alerts be sent either to individal Bricolage users, or to
...
change those values, click the "Edit" button for the particular type of conact.
...
User Group soleley for the purpose of selecting that group for sending alerts of
...
on My Workspace, on a desk, or in any Manager. The fourtch column for each event
...
Doing so bring up the "Alerts" manager. This interface lists all the alerts for
a given event, including the time the were sent, their subject, and their
...
$Log: Alert.pod,v $
Revision 1.1  2001-09-06 21:52:48  wheeler
Initial revision
