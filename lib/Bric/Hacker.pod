...
If you've just created the branch, or if you haven't already checked out the
existing branch, do so now. You'll need it to fix bugs and release future
minor releases of Bricolage, anyway, so it's good to have on hand.

  % svn co cp http://svn.bricolage.cc/bricolage/branches/rev_1_8

Now create the distribution tarball.
