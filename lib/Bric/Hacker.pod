...
Patches should be generated using a C<cvs diff -uw> command on each of
...
diff by running this command from the root of your CVS checkout:
  ...
  cvs diff -uw lib/Bric/Changes.pod comp/foo.mc > patch.txt
...
patch.txt with:
  ...
  diff -uw /dev/null inst/upgrade/1.9.1/solve_fermat.pl >> patch.txt
...
this:
  ...
  File: README            Status: Up-to-date
     ...
     Sticky Tag:          Release_1_4_0 (branch: 1.16.2)
     Sticky Date:         (none)
     Sticky Options:      (none)

     Existing Tags:
          Release_1_4_0                   (branch: 1.16.2)
          Release_1_4_0_merge-2002-08-31  (revision: 1.16.2.1)
          Dev-Release_1_3_3               (revision: 1.10)
          Release_1_2_merge-2002-04-03    (revision: 1.2.2.4)
          Release_1_2_3                   (revision: 1.2.2.4)


From this output, you can see that, for this branch (Release_1_4_0), the file
was tagged with the merge tag "Release_1_4_0_merge-2002-08-31". Thus we can
tell that the branch was last merged on August 31, 2002. If there had not been
a tag with the same version number as the branch, then we could determine that
it hadn't been merged before.
...
from HEAD, make sure it's fully up-to-date, and do a simple merge:
  ...
  cvs -z3 -q update
  cvs -z3 -q update -kk -j Release_1_4_0

Otherwise, do a merge from the most recently-dated merge tag. In the above
example, that would be "Release_1_4_0_merge-2002-08-31".

  cd bricolage-head-checkout/
  cvs -z3 -q update
  cvs -z3 -q update -kk -j Release_1_4_0_merge-2002-08-31 \
    -j Release_1_4_0
...
only from this date on.
  ...
  cd bricolage-branch-checkout/
  cvs -z3 -q tag Release_1_4_0_merge-CCYY-MM-DD
...
tarball. First, tag the release in CVS:
  ...
  cvs -z3 tag Release_1_x_x

If this is a major release, you'll need to create a new branch, so that it
can be maintained for bug fixes separately:

  cvs -z3 tag -b Release_1_x
...
option with C<export> so that the version numbers are all properly populated.
  ...
  cd /tmp
  cvs -z3 export -r Release_1_x_x -kkv bricolage
  cd bricolage
  make dist
