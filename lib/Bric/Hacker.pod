...
Currently there is a /usr/local/www/bin/update_devel_docs and a cron
job set up to run it every night again (using David's personal
account, not root or nobody). To get it to work, the pod_browser was
...
bricolage-1.10.3.tar.gz .
tar zxf bricolage-1.10.3.tar.gz
cd zxf bricolage-1.10.3
/usr/local/bin/pod_browser --app-root . --app-name Bricolage \
  --doc-root /usr/local/www/doc_roots/bricolage.cc/docs/1.10/api \
  --base-uri /docs/1.10/api --version-in Bric.pm

That's it.

There is a symlink /usr/local/www/doc_roots/bricolage.cc/current to
1.10. After a major release (1.12, 2.0, etc.), the symlink would need
to be updated and /ui/js/tree.js would also need to be modified on
