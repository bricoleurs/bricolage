...
We strongrly recommend that you use C<cperl-mode> while editing Bricolage
...
settings, as well:
  ...
  (append '(("\\.\\([pP][Llm]\\|al\\|pod\\)$" . cperl-mode))  auto-mode-alist ))
...
will parse Mason component files and use <sgml-mode> in HTML spaces and
...
component files:
  ...
  (add-to-list 'auto-mode-alist '("/usr/local/bricolage/comp" . html-mode))
  (mmm-add-mode-ext-class 'html-mode "/usr/local/bricolage/comp" 'mason)
...
files by adding this to your C<~/.emacs> file:
  ...
  (add-to-list 'auto-mode-alist '("/usr/local/bricoloage/data/obj" . cperl-mode))
...
If you're working on a performance problem, considering setting
DBI_PROFILE and using bric_dbprof to anaylze database query
performance.
