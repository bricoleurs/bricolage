...
Patches should be generated using a C<cvs -z3 diff -u> command on each of the
...
running this command from the root of your CVS checkout:
  ...
  cvs -qz3 diff -u lib/Bric/Changes.pod comp/foo.mc > patch.txt
...
you would add this to patch.txt with:
  ...
  diff -qz3 -u /dev/null inst/upgrade/1.9.1/solve_fermat.pl >> patch.txt
...
Make sure you check the results with C<cvs -qz3 diff> before committing.
...
example, that would be "rev_1_4_merge-2002-08-31".
  ...
  cd bricolage-head-checkout/
  cvs -qz3 update
  cvs -qz3 update -d -kk -j rev_1_4_merge-2002-08-31 -j rev_1_4
...
only from this date on.
  ...
  cd bricolage-branch-checkout/
  cvs -qz3 tag rev_1_4_merge-CCYY-MM-DD
