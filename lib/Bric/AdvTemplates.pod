...
take advantage of the convenience interface priveded by XML::Writer.
...
site. This is one of the great strenghts of Bricolage's templating architecture,
...
every time they're run, the'll load the template into a separate Apache process,
and therefor use more system resouces (memory). It would be better to load all of
the modules you'll need at Bricolage startup time, so that they get shared across
processes and therefor use less memory.
...
this functionality isn't availabe for HTML::Template templates at this time). So
