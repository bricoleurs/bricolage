...
A template developer would like that all code will be syntax checked, 
have some special debug messages to appear to the story editor previewing
the article; have some special code that only runs at publish time; have
some cookie handling code that only runs in realtime, when an actual
