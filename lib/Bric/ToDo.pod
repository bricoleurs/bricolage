...
Add check for uniqueness of URIs when Stories and Media are created.
Also check that secondary URIs created by adding categories to Stories
are unique.  This could be made optional but should be on by default.
