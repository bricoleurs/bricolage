...
Create a framework for unit testing using Test::Class.
...
Add a check that a user has selected at least one output channel when creating
a new story type element or media type element. Top-level elements without
output channels cause Bricolage indigestion.
...
Make URI format output channel-specific. Include an option to allow the slug
to be the file name, e.g., F<slug.html>, F<slug1.html>, etc.. Media files will
continue to retain their own names, however.

=item *

Chage output channels such that, when they're associated with a story type (or
media type), they will be output channels to which a story of that type I<may>
be published. Each output channel has an associated checkbox indicating
whether a story of that type is published to the given output channel by
default. In the story profile, you see this list, and check a checkbox for
those output channels you want the story published to.
...
Modify JavaScript and CSS to work outside of Mason, that is, without
server-side client identification. This way, the script files can just be sent
to the browser once, rather than re-executing the Mason versions of these
files for every request. Note that this is how the CSS for the help pages
currently works, although their reliance on a JavaScript browser sniffer can
be removed.
