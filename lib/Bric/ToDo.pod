...
C<list()> in all classes. C<Limit> and C<Offset> can be added by using derived
tables (Stephan Szabo is my hero for suggesting this -- I didn't even know it
was possible!). Here's a quick example for person objects:
