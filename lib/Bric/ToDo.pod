...
Add POD documentation to F<bric_dist_mon>, F<bric_ftpd>, and F<bric_pgimport>.
...
Modify C<< Grp->get_objects() >> to use use the C<grp_id> parameter to
C<list()>, once support for that parameter has been added to all classes. Go
...
L<http://bricolage.cc/docs/design/ElementRevision/>.
...
L<http://techdocs.postgresql.org/techdocs/fulltextindexing.php> for one
approach using PostgreSQL.
