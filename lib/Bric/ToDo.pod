...
Estimated ship date: 8 December 2001.
...
Add URI to all media profile views [Michele].

=item *

Add ability to change the category association in Templates [David].
...
Add Context-sensitive, online help. Note that this has been done, but all of the
help files should be proof-read for accuracy and they still need some style
applied to them. [Sam]

=item *

Add separate interface for editing templates (via FTP). Note that this is done,
but needs testing. [Sam]

=item *

Add the ability to use HTML::Template Templates without breaking the use of
Mason. Note that this is done, but needs testing. [Sam]

=item *

Separate the filesystem destinations for stories burned for publication and for
stories burned for previewing. This will prevent someone previewing and stomping
all over a published version of a story. [David]
...
Add support for Mozilla/Netscape 6. Essentially, the JavaScript needs to be
updated.
